...
models <- models[!grepl(paste0("final_fit_black_spruce"), names(models))]#remove black spruce for now
...
for (spp in names(models)){
  ...
  for (i in seq_along(files)){
    file <- files[[i]]
    year <- strsplit(basename(file), "-")[[1]][3]
    
    #one raster layer for spatial reference
    r <- add_xy_bands(file)
    template <- r[[1]]
    
    #conditional for GEE tiles outside AOI
    r_df <- as.data.frame(r)
    if (nrow(r_df) == 0) {
      warning(paste("Skipping file (empty raster):", basename(file)))
      next
    }
    
    #predict onto raster and update the template with prediction values
    prediction <- predict(model, r_df)
    ...
    outname <- paste0(path, "prediction_tile_", pattern, "_", i, "_", year, ".tif")
    
    writeRaster(prediction_raster, filename = outname, overwrite=TRUE)
