...
files <- files[!grepl("-2000-", files)]
files <- files[!grepl("0000005632-002", files)]
```
...
for (f in files) {
  ...
  if (inherits(r, "try-error")) {
    warning(paste("Skipping corrupt raster:", basename(f)))
    next
  }
  if (ncell(r) == 0 || all(is.na(values(r)))) {
    warning(paste("Skipping empty or NA raster:", basename(f)))
    next
  }
