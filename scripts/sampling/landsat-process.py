...
AK_landscape = ee.FeatureCollection('projects/ee-vegshiftsalaska/assets/Tanana_RB_Sampling/tanana_rb') 
...
# %% generate list of years for sampling
dir = "/Users/wancher/Repos/AlaskaVegg/data/processed/csv/"
files = os.listdir(dir)

# extract unique years from file names
years = sorted({
    int(re.search(r'\d{4}', f).group())
    for f in files if re.search(r'\d{4}', f)
})
years = ee.List(years)

# %% generate list of years for image composites (for predicting on)
...
def process_mtbs(AK_landscape, year):
    ...
    # MTBS
    def rename_burn_band(img):
    #rename any band to severity
        return img.select([0], ['severity'])
...
for i in range(year_info):
    ...
    task = ee.batch.Export.image.toDrive(
        ...
        description=f'Tanana-tile-{year}',
    ...
    # cafiPlots = ee.FeatureCollection(f'projects/ee-vegshiftsalaska/assets/Tanana_RB_Sampling/biomass_{year}')
    # def sample_pixels(f):
    ...
    #   description=f'TananaRB_PixelValues-{year}',
